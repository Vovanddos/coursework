...
//        $s=$request->s;
//        $posts= Post::where('title', 'LIKE'," %{$s}%")->paginate(3);
//        $posts = Post::with('category', 'user')->orderBy('created_at', 'DESC')->paginate(3);
//        return view('website.search', compact(['posts']));


        $key = trim($request->get('s'));
...
//            ->orWhere('description', 'like', "%{$key}%")
    ...
    public function send_message(Request $request)
    {
        ...
        Session::flash('message-send', 'Сообщение успешно отправлено');
